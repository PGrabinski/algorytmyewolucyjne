...
ground_truth = str([int(float(x)) for x in ground_truth.split()][:samples])
print(ground_truth)
with open('/workspaces/pyneurgen/2/satelite_data/ImageRawReduced.txt', 'r') as data_file:
    satelite_images = data_file.read()
satelite_images = str([[float(x) for x in line.split()][:samples] for line in satelite_images.splitlines()])
...
rules = [[<r11>, <r12>, <r13>], [<r21>, <r22>, <r23>], [<r31>, <r32>, <r33>]]
...
ges.set_max_program_length(50000)
