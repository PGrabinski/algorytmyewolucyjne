...
expert_file = open('/workspaces/pyneurgen/2/satelite_data/ImageExpertReduced.txt', 'r')
ground_truth = expert_file.read()
ground_truth = np.array([int(float(x)) for x in ground_truth.split()])
expert_file.close()

data_file = open('/workspaces/pyneurgen/2/satelite_data/ImageRawReduced.txt', 'r')
